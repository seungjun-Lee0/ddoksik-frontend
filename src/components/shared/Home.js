  ...
  const handleDeleteMealPlan = async (mealId) => {
    if (window.confirm("Are you sure you want to delete this meal plan?")) {
      try {
        ...
        if (response.ok) {
          // Filter out the deleted meal plan from the groupedDiets state
          ...
          alert("Failed to delete the meal plan.");
        }
      } catch (error) {
      ...
      <div className="searched-jobs">
        <div className="searched-bar">
          ...
          <div className="searched-sort" onClick={toggleSortOrder}>
            Sort by: <span className="post-time">{sortOrder === 'newest' ? 'Newest Post' : 'Oldest Post'} </span>
